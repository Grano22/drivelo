...
export class RentedCarListComponent {
    ...
    protected returnCar(rentalId: string): void {
        if (!confirm('Are you sure you want to return this car?')) {
            return;
        }

        this.#carFleetInternalHttpClients.returnCar(rentalId).subscribe({
            next: async response => {
    ...
    #initializeData(): void {
        this.#carFleetInternalHttpClients.getRentedCars().subscribe({
            next: (cars) => {
                this.store.setRentedCars(cars);
            },
            error: (error) => {
