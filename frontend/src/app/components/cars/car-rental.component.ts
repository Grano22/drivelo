...
export class CarRentalComponent {
    ...
    readonly #initialEndDate = new Date(Date.now() + 24 * 60 * 60 * 1000);
    ...
    protected selectedMinDate = signal<Date>(this.minDate);
    protected selectedMaxDate = signal<Date>(this.#initialEndDate);
    ...
    protected readonly totalDays = computed(() => {
        const startDate = this.selectedMinDate();
        const endDate = this.selectedMaxDate();
    ...
    constructor() {
        ...
        this.rentalForm.get('startDate')?.valueChanges.subscribe(() => {
            ...
            this.selectedMinDate.set(this.rentalForm.get('startDate')?.value || this.minDate);
        });

        effect(() => {
            this.rentalForm.get('endDate')!.valueChanges.subscribe(() => {
                this.selectedMaxDate.set(this.rentalForm.get('endDate')?.value || this.#initialEndDate);
            });
        });
    }
